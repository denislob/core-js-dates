...
function getNextFriday(date) {
  ...
  const remainingDays = currentDay < 5 ? 5 - currentDay : 5 + (7 - currentDay);
  const nextFriday = new Date(currentDate);
  nextFriday.setDate(currentDate.getDate() + remainingDays);
...
function getCountDaysInMonth(month, year) {
  ...
  const countDaysInMonth = lastDayCurrentMonth.getDate();
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const alpha = new Date(dateStart);
  const omega = new Date(dateEnd);
  const delta = omega - alpha;
  const daysDelta = Math.ceil(delta / (1000 * 3600 * 24));
  const countDaysOnPeriod = daysDelta + 1;
  return countDaysOnPeriod;
}
...
function isDateInPeriod(date, period) {
  ...
  const beginningPeriod = new Date(period.start);
  const finalPeriod = new Date(period.end);
  return searchedDate >= beginningPeriod && searchedDate <= finalPeriod;
}
...
function formatDate(date) {
  ...
  const noonOrMidnight = hours >= 12 ? 'PM' : 'AM';
  hours %= 12;
  hours = hours === 0 ? 12 : hours;
  const processedMinutes = minutes.toString().padStart(2, '0');
  const processedSeconds = seconds.toString().padStart(2, '0');
  return `${month}/${day}/${year}, ${hours}:${processedMinutes}:${processedSeconds} ${noonOrMidnight}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const firstDayOfYear = new Date(dCopy.getFullYear(), 0, 1);
  const weekNumber = Math.ceil(((dCopy - firstDayOfYear) / 86400000 + 1) / 7);
...
function getNextFridayThe13th(date) {
  ...
  actualDate.setDate(13);
  if (actualDate <= date) {
    actualDate.setMonth(actualDate.getMonth() + 1);
    actualDate.setDate(13);
  }
  while (actualDate.getDay() !== 5) {
    actualDate.setMonth(actualDate.getMonth() + 1);
    actualDate.setDate(13);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const arrOfWorkingDaysDates = [];
  const [beginDay, beginMonth, beginYear] = period.start.split('-').map(Number);
  const [finalDay, finalMonth, finalYear] = period.end.split('-').map(Number);
  const beginDate = new Date(beginYear, beginMonth - 1, beginDay);
  const finalDate = new Date(finalYear, finalMonth - 1, finalDay);
  const actualDate = new Date(beginDate);
  let isWeekDay = true;
  let numberOfWeekdays = 0;
  let numberOfWeekend = 0;

  while (actualDate <= finalDate) {
    if (isWeekDay) {
      if (numberOfWeekdays < countWorkDays) {
        const day = String(actualDate.getDate()).padStart(2, '0');
        const month = String(actualDate.getMonth() + 1).padStart(2, '0');
        const year = actualDate.getFullYear();
        arrOfWorkingDaysDates.push(`${day}-${month}-${year}`);
        numberOfWeekdays += 1;
        if (numberOfWeekdays === countWorkDays) {
          isWeekDay = false;
          numberOfWeekend = 0;
      ...
      numberOfWeekend += 1;
      if (numberOfWeekend === countOffDays) {
        isWeekDay = true;
        numberOfWeekdays = 0;
      }
    }
